...
def return_xlsx_folder_path(data_axis_path, xlsx_folder_relative_path):
    xlsx_folder_full_path = data_axis_path + xlsx_folder_relative_path
    return xlsx_folder_full_path

def return_xlsx_path(data_axis_path, xlsx_name, xlsx_folder_relative_path):
    xlsx_full_path = return_xlsx_folder_path(data_axis_path, xlsx_folder_relative_path) + xlsx_name
    return xlsx_full_path

def return_db_path(data_axis_path, db_name):
    db_full_path = data_axis_path + db_name
    return db_full_path

def return_df_db_dict(versus_db_df):
    versus_df_db = return_swap_dict(versus_db_df)
    return versus_df_db
